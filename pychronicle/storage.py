...
class TraceStore:
    ...
    def _create_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE INDEX IF NOT EXISTS changes_frame
                ON changes(frame_id);

            CREATE INDEX IF NOT EXISTS changes_name
    ...
    def record(
        ...
        encoded = {
            name: self.serialize(value)
            for name, value in values.items()
        }

        prior_names = {
            name
            for candidate_scope, name in self._last
    ...
    def state_at(
        ...
        for row in self.connection.execute(
            ...
            name = (
                row["variable_name"]
                if scope
                else f"{row['scope']}::{row['variable_name']}"
            )

            if row["operation"] == "delete":
                state.pop(name, None)
            else:
                state[name] = json.loads(
                    row["value_json"]
                )
