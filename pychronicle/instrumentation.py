    ...
    def _add(
        ...
        if names:
            self.assignments.append(
                Assignment(node.lineno, names, kind)
            )

    def visit_Assign(self, node: ast.Assign) -> None:
        self._add(node, node.targets, "assign")
        self.generic_visit(node)

    def visit_AnnAssign(self, node: ast.AnnAssign) -> None:
        self._add(node, [node.target], "annotated")
        self.generic_visit(node)

    def visit_AugAssign(self, node: ast.AugAssign) -> None:
        self._add(node, [node.target], "augmented")
        self.generic_visit(node)

    def visit_For(self, node: ast.For) -> None:
        self._add(node, [node.target], "loop")
        self.generic_visit(node)
...
class CheckpointInjector(ast.NodeTransformer):
    """Append a checkpoint hook after ordinary statements."""
    ...
    def _hook(self, statement: ast.stmt) -> ast.Expr:
        call = ast.Call(
            func=ast.Name(
                id="__pychronicle_checkpoint__",
                ctx=ast.Load(),
            ),
            args=[
                ast.Constant(statement.lineno),
                ast.Call(
                    func=ast.Name(
                        id="locals",
                        ctx=ast.Load(),
    ...
    def _body(
        ...
        for statement in statements:
            ...
            if (
                ...
                and not isinstance(statement, self.excluded)
            ):
    ...
    def visit_Match(self, node: ast.Match) -> ast.Match:
        """
        Instrument every case body inside a match statement.
        This ensures assignments inside case blocks receive
        checkpoint hooks.
        """
